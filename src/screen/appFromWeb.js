var app = {
		...
		if(activePage.id == 'channel'){
			...
			if($('.netflix_popup_opened').length == 0){
				...
				if($('#cincopa_'+app.current_channel_fid).hasClass('status-pause')){
					...
					if($(controls).hasClass('ze-active-controls') ) {
						app.playerAPI().pause();
					}else {
	...
	controlsClick: function (id,keyCode) {
		...
		if(app.Nav.isOpen() && selectedItem.length == 0 ){
			...
			selectedItem[0].scrollIntoView({behavior:'smooth',block: 'start'});
	...
	getUsersList: function(userId){
		...
		$.each(app.users,function(i,data){	
			...
			<div>'+data.vendor_name+'-'+data.name + (data.status=="expired"? '<div class="showMessage">expired</div>' : '<div class="showMessage"></div>') +' </div> <i class="channels-list__remove zmdi zmdi-delete"></i></div>';
			index++;
		});
	...
	drawChannel:function(fid,channelName){
		...
		$('.channel').html('');
		var newEmbedCode = `<div id="cincopa_${fid}" class='gallerydemo cincopa-fadein'>
									<div class="embedimageContainer" style='width: 100%; height: 100vh; max-width: 100%;'>
										<img src='https://rtcdn.cincopa.com/thumb.aspx?fid=${fid}&size=medium' style='filter:blur(5px);heiXght:100%;object-fit:cover;width:100vw;height: 100vh' onload='this.parentNode.style.opacity=1;' />
									</div>
							</div>
	...
	drawChannels:function(galleryId,galleries){
		...
		<div class="channel-list__title"><h3>${app.users[galleryId].name}</h3></div>
		<div class="channel-list__descr"><p>${app.users[galleryId].vendor_description}</p></div>
		</div>
    ...
    generateLastView:function(lastSeenVideos){
		...
		$.each(lastSeenVideos, function(key,lastSeen){	
			...
			htm += `<div class="channel-list__lastView--item" 
			data-top="scrollup"
